...
def get_scheduler(
    ...
    if method == "StepLR":
        ...
        scheduler = optim.lr_scheduler.OneCycleLR(
            optimizer,
            max_lr=10 * optimizer.param_groups[0]["lr"],
...
def load_criterion_state(
    criterion: Union[nn.Module, List[nn.Module]],
    checkpoint: Union[Dict[str, Any], None],
) -> Union[nn.Module, List[nn.Module]]:
    """
    Loads the state of one or multiple loss functions from the provided checkpoint.

    Args:
        criterion (Union[nn.Module, List[nn.Module]]): The loss function(s) to load the state into.
        checkpoint (Union[Dict[str, Any], None]): The checkpoint dictionary containing the loss states.

    Returns:
        Union[nn.Module, List[nn.Module]]: The criterion(s) with the loaded state dict (if available).
    """
    if checkpoint:
        try:
            if isinstance(criterion, list):
                if len(criterion) != 2:
                    raise ValueError(
                        "Expected a list with exactly 2 criteria (pose & depth)."
                    )

                pose_criterion, depth_criterion = criterion

                if "criterion_pose_state_dict" in checkpoint:
                    pose_criterion.load_state_dict(
                        checkpoint["criterion_pose_state_dict"]
                    )
                    print("--- Pose criterion state loaded successfully.")

                if "criterion_depth_state_dict" in checkpoint:
                    depth_criterion.load_state_dict(
                        checkpoint["criterion_depth_state_dict"]
                    )
                    print("--- Depth criterion state loaded successfully.")

            elif isinstance(criterion, nn.Module):
                if "criterion_state_dict" in checkpoint:
                    criterion.load_state_dict(checkpoint["criterion_state_dict"])
                    print("--- Single criterion state loaded successfully.")
                else:
                    print("--- No 'criterion_state_dict' found in checkpoint!")

            else:
                raise ValueError(
                    "Criterion must be either a single nn.Module or a list of two nn.Module objects."
                )
        ...
        print(
            "--- No checkpoint provided. Criterion initialized with default parameters."
        )
...
def update_training_params(
    ...
    if checkpoint is not None:
        try:
            ...
            training_params["epoch_init"] = checkpoint.get("epoch", 1) + 1
