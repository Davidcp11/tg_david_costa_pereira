...
from models.build_model import load_model_state, initialize_heads_with_xavier
from utils.train_utils import (
    load_optimizer_state,
    load_criterion_state,
...
def load_checkpoint(checkpoint_fpath: Union[str, Path, None]) -> Optional[Dict]:
    ...
    # Load checkpoint and the model's state dict (map to available device if CUDA not present)
    map_location = None if torch.cuda.is_available() else torch.device("cpu")
    checkpoint = torch.load(checkpoint_fpath, weights_only=True, map_location=map_location)
...
def update_state(
    ...
    same_exp = (
        checkpoint_fpath.parent.name == checkpoint_params["checkpoint_dpath"].name
    )
    if same_exp and ("checkpoint_last.pth" in str(checkpoint_fpath)):
    ...
    if restart_training:
        ...
        # Update criterion state (if there are trainable parameters)
        criterion = load_criterion_state(criterion, checkpoint)
        ...
        # Random initialization of the head
        initialize_heads_with_xavier(model)
