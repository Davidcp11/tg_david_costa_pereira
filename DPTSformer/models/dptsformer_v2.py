...
from models.blocks_v2 import ReassembleBlock, FusionBlock
...
class DPTSformer(nn.Module):
    ...
    def __init__(
        ...
        attention_type="divided_space_time",
        dropout=0.0,
        pretrained_dpt_path="models/dpt/weights/dpt_hybrid-midas-501f0c75.pt",
        dpt_backbone="vitb16_384",
        dpt_features=256,
        vit_layers=[0, 1, 2, 3],
        dpt_readout="proj",
        use_bn=False,
        ...
        # Fusion blocks
        self.fusion_1 = FusionBlock(resample_dim=dpt_features)
        self.fusion_2 = FusionBlock(resample_dim=dpt_features)
        self.fusion_3 = FusionBlock(resample_dim=dpt_features)
        self.fusion_4 = FusionBlock(resample_dim=dpt_features)
    ...
    def forward_features(self, x):
        ...
        for i in range(self.depth):
            ...
            if i in self.vit_layers:
                B, N, E = x.shape
                H = int((N - 1) / (W * T))
                feat = rearrange(x, "b (n t) m -> (b t) m h w", b=B, h=H, w=W)
                features.append(x)
